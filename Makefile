...
build-flink:
	@if [ -f flink-jobs/pom.xml ]; then \
		mvn -q -f flink-jobs/pom.xml clean package; \
	else \
		echo "Phase 1.1 scaffold only: Flink job pom is introduced in Phase 1.2."; \
		exit 2; \
	fi

submit-flink:
	@echo "Phase 1.1 scaffold only: Flink CDC job is introduced in Phase 1.2."
	@exit 2
...
restore:
	...
	@echo "Phase 1.1 scaffold only: restore target is wired in Phase 1.2 with the job jar."
	@exit 2
...
eo-verify:
	@echo "Phase 1.1 scaffold only: eo-verify is introduced in Phase 1.2."
	@exit 2
...
test-cdc:
	$(PYTHON) -m pytest harness/tests/cdc_correctness -v
...
lint:
	...
	@if [ -f flink-jobs/pom.xml ]; then \
		mvn -q -f flink-jobs/pom.xml verify; \
	else \
